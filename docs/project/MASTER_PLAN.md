...
* **Step 2.5A: SEC EDGAR Foreign-Private-Issuer Annual-Filing Coverage** `[CURRENT — A0 REVIEW]`: D0 and Gate A are complete. Bounded A0 is implemented and verified: exact ticker-to-CIK identity is preserved, multi-ticker CIKs no longer erase issuer-level facts, and per-share use remains fail-closed. Work is stopped for review before A1. Gate A also approved resolver-spanning snapshot ownership for later B1-A. Later slices add foreign annual forms to existing US-GAAP duration concepts, then a narrow exact IFRS duration mapping for diluted EPS, adjusted weighted-average shares, operating cash flow, and physical-PP&E CapEx. IFRS BVPS, absence-based preferred-zero inference, ADR/ADS conversion, currency conversion, custom extensions, and broad concept fallback remain deferred.
