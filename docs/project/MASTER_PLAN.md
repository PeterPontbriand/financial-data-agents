...
Step 2.3 established Graham's method, input-resolution, production-provider, and terminal-presentation foundations. Step 2.4 adds a third deterministic cash-flow/growth strategy to exercise and minimally extend those foundations, then closes the bounded Graham/shared-contract hardening discovered during design-to-implementation reconciliation. Step 2.5 evaluates the resulting stable v0.2 deterministic strategy/tool contracts. Step 3.1 adds durable production persistence/cache. Step 3.4 adds the local research workspace: watchlists, user-initiated concurrent refresh, and a durable Analysis Run library. Step 3.5 adds the deterministic quantitative screening suite. Step 3.6 validates the complete Light Mode investor workflow (now including the Step 3.5 screens) and adds only bounded local-model synthesis over already-computed typed results.
...
* **Step 3.1: SQLite DB & Migration Infrastructure** `[COMPLETE / APPROVED]`: Set up Alembic, enforce WAL mode, establish market-data and trajectory storage, add `SQLiteTrajectorySink`, and implement durable cache/persistence behind the historical-price and financial-fact contracts established in Steps 2.3–2.4. Gate D0, Slices A/B1/B2/B3/C1/C2/D1/D2/E1/E2/F1/F2/G, and Gates B/C/D/E/G are approved. The project owner approved Slice G on 2026-09-06, closing Gate G and completing Step 3.1. The complete gate passed with 1,615 tests and 88% coverage. JSONL remains the default. Later work follows the selected sequence below; Step 3.2 is complete and approved on 2026-09-07.
* **R1: Analyzer Separation & CLI Support — complete and approved on 2026-09-06.** All three review gates are closed. The method-specific analyzers/configs, separate Graham commands, shared CLI support, migration documentation, and regression coverage passed the final managed gate (1,809 tests, 89% coverage, Ruff, formatting, and strict mypy). R1 preceded Step 3.4 by sequencing choice, not a technical dependency. R1 and R2 were merged together in [PR #29](https://github.com/PeterPontbriand/financial-data-agents/pull/29) on 2026-09-07; their implementation/PR workflows are closed.
* **P2-Profiles: Durable Instrument Profiles — after Steps 3.2–3.3, before Step 3.4:** Persist time-aware identity/classification behind the existing profile seam with reviewed keys, provenance, freshness, invalidation, provider precedence, and ticker-reuse rules. Analysis Runs preserve the exact execution snapshot. No ETF holdings or aggregation work is included.
* **P2-ETF: ETF Aggregate FCF Growth — deferred until after Step 3.6:** Add a separate look-through ETF strategy only when subsequently prioritized and explicitly authorized, following its provider-evidence/product-policy gate. It owns holdings, weights, currency, coverage, and historical semantics; it must not replace an explicitly requested company-level analysis. This deliverable is not required for Steps 3.4–3.6 or the v0.2.5 validation checkpoint.
* **Step 3.2: DAO & Repository Layer** `[COMPLETE / APPROVED]`: Started on 2026-09-07 following R2 completion. Completed the reviewed gaps: bounded cache-key enumeration, stored-fact inspection, and trajectory SQL extraction behind a typed repository. Existing storage representations, cache eligibility, telemetry behavior, and connection policy are preserved. All gates are approved; final Gate 3.2-D approval on 2026-09-07 completes the step. The accepted final managed gate passed 1,853 tests with 89% reported coverage; see [Step 3.2 Contract and Slice Plan](milestones/v0.2/step-3.2/STEP_3_2_CONTRACT_AND_SLICE_PLAN.md).
* **Step 3.3: Data Quality & Cache Invalidation Pipeline** `[COMPLETE / APPROVED]`: Validate incoming financial data (currency consistency, corporate actions where applicable, continuity/missing data, and staleness) and apply controlled cache refresh/invalidation rules. A reconciliation and a fresh green baseline are complete; B pure rules and focused tests are implemented (Gate B review passed on 2026-09-08); C cache/refresh and trajectory integration is implemented and wired into the audited fetch/cache and resolver paths. The accepted final managed gate passed 1,944 tests with 89% reported coverage on 2026-09-08; final acceptance is recorded in [Implementation Plan §4.9](milestones/v0.2/IMPLEMENTATION_PLAN.md#49-step-33--data-quality--cache-invalidation-pipeline). See the [Step 3.3 Contract and Slice Plan](milestones/v0.2/step-3.3/STEP_3_3_CONTRACT_AND_SLICE_PLAN.md).
* **Existing Strategy Correctness Audit and Repair** `[PLANNING APPROVED / ACCEPTANCE OPEN]`: Audit and repair Graham Number, Graham Growth, Momentum and FCF/Earnings Growth before database readiness resumes. The [correctness plan](milestones/v0.2/existing-strategy-correctness/EXISTING_STRATEGY_CORRECTNESS_PLAN.md) owns the coverage matrix, defect ledger and ESC-A–D gates. Reopen and fold Graham R2/R3 final acceptance into ESC-C. Require no unresolved known output-correctness defects, independently checked math, realistic composition regressions and dated live evidence. Renew acceptance at ESC-D immediately before Step 3.5; further strategy development is blocked until discovered defects are fixed and reviewed.
* **Step 3.3A: Fresh Database Initialization & Schema Readiness** `[PLANNING]`: Before Step 3.4, initialize only verified fresh SQLite storage through Alembic; preserve explicit existing-schema upgrades; provide typed actionable readiness failures and deterministic lifecycle/concurrency/rollback tests. See the [companion plan](milestones/v0.2/step-3.3a/STEP_3_3A_CONTRACT_AND_SLICE_PLAN.md). Design/implementation continuation awaits ESC-C acceptance and its own contract approval. Step 3.4 start authorization is retained but deferred until final acceptance.
...
* **Step 3.5: Deterministic Quantitative Screening Strategies**  
  Entry requires explicit ESC-D renewed acceptance of all four existing analyses on the starting revision; no unresolved known output-correctness defect may be carried into new strategy implementation.

  Add a bounded suite of five classic, independently typed, fully deterministic quantitative screening strategies that operate exclusively on free/public data (SEC EDGAR XBRL primary, `yfinance` fallback) and reuse the existing financial-fact, InputResolver, provenance, point-in-time, and Analysis Run contracts:

  1. **Piotroski F-Score** — full 9-point binary fundamental quality score.
  2. **Altman Z-Score** — classic 5-factor insolvency model (non-financial issuers only; explicit inapplicability for banks/insurers/REITs).
  3. **Beneish M-Score** — 8-variable earnings-manipulation detector (probability flag, not a binary label).
  4. **Unlevered Valuation Multiples** — EV/EBITDA and FCF Yield, reusing the canonical Step 2.4 `FCF = CFO − CapEx` definition.
  5. **Greenblatt Magic Formula Ranking** — dual-factor (ROC + Earnings Yield) percentile ranking over an explicit caller-supplied universe (watchlist or ticker list).

  Each strategy is a first-class `BaseAnalyzer` under `src/analysis/strategy/`, produces method-explicit results with progressive disclosure, carries full provenance, and receives its own user-facing strategy guide. No paid data sources, no LLM arithmetic, and no silent imputation of missing values. Golden-Suite extension is required before Step 3.6 close-out. See the companion Step 3.5 plan for full technical notes, edge-case rules, and implementation sequence.

* **Step 3.6: Light Mode Support (required before v0.2.5)**  
  * **Goal:** Ensure the complete investor workflow—data fetch/cache → deterministic analytics (including the Step 3.5 screens) → stored Analysis Run → concise/detailed inspection → bounded local-model synthesis—runs cleanly under Light Mode with a 14B-class (or smaller) model.  
  * **Deliverables:** Configuration defaults favor Light Mode; README and `docs/user/HARDWARE.md` provide the supported workflow; basic smoke tests pass; Step 2.2 empirical schema/model compatibility is recorded; a simple `analyze TICKER` workflow may combine the default deterministic analyses (now including selected Step 3.5 screens) and optionally synthesize their already-computed typed results.  
  ...
  * **Exit criterion:** A new user following only the Light Mode instructions can add or analyze a real ticker, run/refresh supported analyses (including the Step 3.5 suite), inspect a concise result and its provenance, revisit completed Analysis Runs, and obtain bounded synthesis without developer intervention.

**Selected sequence:** R1 and R2 (complete, approved, and merged in [PR #29](https://github.com/PeterPontbriand/financial-data-agents/pull/29)) → Step 3.2 scope reconciliation/completion → Issue #17 telemetry closeout → Step 3.3 → Existing-strategy correctness audit/repair and ESC-C acceptance → Step 3.3A → Step 3.4 → P2-Profiles → ESC-D renewed acceptance → **Step 3.5 Deterministic Quantitative Screening Strategies** → Step 3.6. [R2's final record](milestones/v0.2/r2/R2_CONTRACT_AND_SLICE_PLAN.md#14-final-approval-and-r2-completion) preserves the approved gate evidence (1,811 tests, 89% reported coverage, complete managed gate). Step 3.2 planning started on 2026-09-07; Gates 3.2-A/B/C are approved and the implementation/acceptance gate passed 1,853 tests with 89% reported coverage. The documents-only checkpoint was pushed before implementation. Final Gate 3.2-D approval was granted on 2026-09-07; Step 3.2 is complete. Step 3.2 merged in [PR #30](https://github.com/PeterPontbriand/financial-data-agents/pull/30) as `f3ef25701cac3fbee3a2caa295f102f4d389d51b` on 2026-09-08 at 00:11 UTC (2026-09-07 in America/Toronto). The project owner requested Issue #17 next; its [Telemetry Closeout Plan](milestones/v0.2/issue-17/ISSUE_17_TELEMETRY_CLOSEOUT_PLAN.md) received contract and final acceptance approval on 2026-09-07. Recovery emission and hashing already exist; the bounded regression coverage is complete and approved without production changes. Its commit/PR publication and GitHub issue closure remain outstanding. This ordering is a prioritization decision, not a new technical dependency. The project owner explicitly authorized Step 3.3 to begin on 2026-09-07, clarifying the intended step number; completed Step 2.4 is not reopened. The project owner approved the Step 3.3 companion plan and explicitly authorized implementation on 2026-09-07 on `feat/step-3.3-data-quality`. The [Step 3.3 companion plan](milestones/v0.2/step-3.3/STEP_3_3_CONTRACT_AND_SLICE_PLAN.md) records approval, initial inventory, accepted quality rules, slices, and verification requirements. A reconciliation and a fresh green baseline are complete. B pure quality rules and focused tests are implemented; Gate B review passed on 2026-09-08. C cache/refresh and trajectory integration is implemented and verified, and the full managed quality gate passed 1,944 tests with 89% reported coverage. Step 3.3 is complete and approved on 2026-09-08. P2-Profiles, Step 3.4, Step 3.5, and Step 3.6 remain unstarted; Step 3.4 retains its 2026-09-08 start authorization but is deferred until Step 3.3A final acceptance and still requires its companion contract. Other packages retain separate review/authorization gates; P2-ETF remains deferred beyond 3.6 until separately prioritized.
