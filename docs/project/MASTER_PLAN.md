...
* **Immediate Graham Price Comparison Repair** `[R1 READY FOR REVIEW]`: Before Step 3.3A, restore production ordinary-share comparisons using validated share-unit evidence, retain the compatibility safeguard, and expose specific unavailable reasons in both Graham methods. The [repair plan](milestones/v0.2/graham-comparison/GRAHAM_COMPARISON_REPAIR_PLAN.md) requires evidence/contract review before implementation and final acceptance before readiness work resumes. [R1 evidence and contract](milestones/v0.2/graham-comparison/R1_EVIDENCE_AND_CONTRACT.md) are ready for review with a green baseline; implementation remains unapproved. This is an immediate bounded defect repair, not full P2-Profiles.
