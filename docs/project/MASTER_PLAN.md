...
* **Step 4.1: Additional Fundamental Valuation Multiples & Screening Analyzers:** Add deterministic fundamental and relative-valuation screens using the existing typed analysis and financial-fact boundaries.
  ...
  * **Further candidates:** Subject to separate product-policy approval and data evidence; see the [Evidence Provider Roadmap](EVIDENCE_PROVIDER_ROADMAP.md) for the current candidate list rather than enumerating it here. FCF/share growth is owned by Step 2.4 and may be consumed independently by later aggregation; these candidates do not define a composite method.
