...
* **Step 2.4: Free Cash Flow & Earnings Growth Analysis** `[COMPLETE / APPROVED]`: The third deterministic strategy, E1–E3 FCF/share extension, pre-Golden shared-contract hardening, F-1 shared security-identity presentation work, and Slice G documentation/final-gate closeout are complete and approved as of 2026-08-30. The approved strategy is a historical cash-flow/growth screen using project-defined `FCF = CFO - CapEx`, explicit period alignment and provenance, both total-company-FCF and FCF-per-diluted-share growth, and historical diluted-EPS growth. Total-company-FCF CAGR controls classification by default; an explicit typed policy/CLI override may select FCF/share CAGR instead. Do not silently turn this into DCF, analyst-forecast growth, a broad named-investor methodology, or a composite recommendation score. `docs/project/milestones/v0.2/STEP_2_4_FCF_EARNINGS_GROWTH_DESIGN.md` governs the strategy.
* **Step 2.5: Golden-Test Suite & Strategy Evaluation** `[CURRENT — PAUSED AT GATE M]`: P1 instrument-applicability hardening and the initial typed/evaluator/runner/fixture/case implementation are present at checkpoint `4d08b127`. The 2026-08-31 Gate M review did not approve the checkpoint: strict mypy fails, expected non-success domain outcomes are not consistently enforced by aggregate evaluation, the full minimum suite lacks one canonical report entry point, and the cross-strategy ETF Golden scenario is incomplete. Perform only the bounded Slice H correction, return to Gate M, and keep deterministic/no-LLM regression results separate from optional real-local-Ollama empirical evaluation. Target ≥90% aggregate pass rate without weakening benchmark criteria.
* **Step 2.5A: SEC EDGAR Foreign-Private-Issuer Annual-Filing Coverage** `[PLANNED / BLOCKED BY STEP 2.5]`: After Step 2.5 closeout, add foreign annual forms to existing US-GAAP duration concepts, then a narrow exact IFRS duration mapping for diluted EPS, adjusted weighted-average shares, operating cash flow, and physical-PP&E CapEx. Select accounting regime from the latest eligible annual accession at `as_of` and reuse one immutable SEC snapshot per analysis. Require affirmative ordinary-share/quoted-unit compatibility for per-share or quote comparisons. IFRS BVPS, absence-based preferred-zero inference, ADR/ADS conversion, currency conversion, custom extensions, and broad concept fallback remain deferred.
