...
* **Step 3.1: SQLite DB & Migration Infrastructure** `[D0 MAPPING IN PROGRESS]`: Set up Alembic, enforce WAL mode, establish market-data and trajectory storage, add `SQLiteTrajectorySink`, and implement durable cache/persistence behind the historical-price and financial-fact contracts established in Steps 2.3–2.4. D0 freezes the contract-to-schema mapping before production code or migrations begin.
