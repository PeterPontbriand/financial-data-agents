...
* **Immediate Graham Price Comparison Repair** `[R2 VERIFIED / FINAL REVIEW PENDING]`: R1 and implementation were approved, including the bounded source-lineage extension. Both Graham methods now acquire supported ordinary-share evidence and expose specific unavailable reasons while retaining the compatibility safeguard. [R2 implementation and verification](milestones/v0.2/graham-comparison/R2_IMPLEMENTATION_AND_VERIFICATION.md) records the 2,004-test, 89%-coverage managed gate. The [repair plan](milestones/v0.2/graham-comparison/GRAHAM_COMPARISON_REPAIR_PLAN.md) requires final R2/R3 acceptance before Step 3.3A resumes. This remains a bounded defect repair, not full P2-Profiles.
* **Step 3.3A: Fresh Database Initialization & Schema Readiness** `[PLANNING]`: Before Step 3.4, initialize only verified fresh SQLite storage through Alembic; preserve explicit existing-schema upgrades; provide typed actionable readiness failures and deterministic lifecycle/concurrency/rollback tests. See the [companion plan](milestones/v0.2/step-3.3a/STEP_3_3A_CONTRACT_AND_SLICE_PLAN.md). Implementation awaits contract approval. Step 3.4 start authorization is retained but deferred until final acceptance.
