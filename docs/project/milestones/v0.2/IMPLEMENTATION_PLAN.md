...
| 9 | [Existing-analysis renewal (ESC-D)](existing-strategy-correctness/ESC_D_RENEWAL_PLAN.md) | In progress (full seven-dimension audit-matrix re-run); ESC-18 corrected in the ledger (the affected `DataQualityError` branch was already unreachable dead code, not a real regression — see ledger). Required on the proposed screening starting revision; no unresolved correctness defects. |
