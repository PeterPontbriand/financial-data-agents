...
| 9 | [Existing-analysis renewal (ESC-D)](existing-strategy-correctness/ESC_D_RENEWAL_PLAN.md) | Renewal evidence complete ([final acceptance record](existing-strategy-correctness/ESC_D_FINAL_ACCEPTANCE.md) drafted); pending project-owner final acceptance. Full seven-dimension audit-matrix re-run found and repaired ESC-19; ESC-18 was corrected (its branch was already unreachable dead code, tracked as R3). No unresolved correctness defects. |
| 10 | Repository-wide dead code audit (R3) | Not started; scheduled here deliberately, after ESC-D and before Step 3.5 adds five new analyzers, so the audit covers a smaller codebase than it would afterward. Motivated by ESC-18's correction, which surfaced a genuinely unreachable branch in `src/data/cached_client.py` during ESC-D.4 reconnaissance. |
