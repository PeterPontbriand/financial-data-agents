...
Execute this short set of commands as a smoke test after deploying or updating Investment Analysis Engine. Run the commands from the repository root. They exercise representative CLI parsing, live provider access, deterministic calculations, progressive-disclosure output, and machine-readable output without attempting exhaustive coverage.
...
3. Exercise custom SMA and RSI periods with detailed market-data context:
   ...
   ```powershell
   uv run ian momentum MSFT --short-window 10 --long-window 30 --rsi-period 14 --details
   ```

4. Inspect Momentum diagnostics for a defensive consumer company:

   ```powershell
   uv run ian momentum KO --diagnostics
   ```
...
8. Exercise a historical Graham boundary and its resolver diagnostics:
   ...
   ```powershell
   uv run ian graham-number MSFT --as-of 2025-12-31 --diagnostics
   ```
