"""SEC EDGAR adapter for filing-dated annual diluted EPS facts."""
...
class SecEdgarValuationAdapter:
    """Provide annual diluted EPS observations from SEC Company Facts.

    This adapter intentionally supports only the capability accepted by the
    Step 2.3 E1 evidence gate: fiscal-year diluted EPS.  Other valuation fields
    and EPS bases return an empty tuple (provider-neutral unavailability).
    ...
    def __init__(
        ...
        *,
        ...
        user_agent: str = "financial-data-agents/0.2 github.com/PeterPontbriand/financial-data-agents",
    ) -> None:
        """Initialize the adapter with injectable transport and clock."""
    ...
    def fetch_facts(self, request: ValuationFactRequest) -> tuple[ProviderFact, ...]:
        """Return annual diluted EPS facts, or explicit unavailability."""
        ...
        try:
            ...
            return _select_one_fact_per_period(candidates, request=request, now=self._clock())
    ...
    def _supports(self, request: ValuationFactRequest) -> bool:
        return (
            request.provider_id == SEC_PROVIDER_ID
            and request.subject_kind is ValuationSubjectKind.SECURITY
            and request.field_name is ValuationField.EPS
            and request.basis == "fiscal_year"
